...
class TestControllerAPIRequires(unittest.TestCase):
    ...
    def setUp(self):
        self.car = requires.ControllerAPIRequires('some-relation', [])
        self._patches = {}
        self._patches_start = {}
    ...
    def patch_kr(self, attr, return_value=None):
        mocked = mock.patch.object(self.car, attr)
    ...
    def test_registered_hooks(self):
        ...
        hook_patterns = {
            'changed': (
                ('{requires:odl-controller-api}-relation-'
                 '{joined,changed,departed}'), ),
            'broken': ('{requires:odl-controller-api}-relation-broken', ),
        }
    ...
    def test_changed_no_connection(self):
        self.patch_kr('set_state')
        self.patch_kr('remove_state')
        self.patch_kr('connection', False)
        self.car.changed()
    ...
    def test_connected(self):
        ...
        self.patch_kr('username', 'bob')
        self.patch_kr('password', 'hardpass')
        self.assertEqual(self.car.connection(), {
            'host': '10.0.0.10',
            'port': '1234',
            'username': 'bob',
            'password': 'hardpass'
        })
        self.patch_kr('password', None)
        self.assertEqual(self.car.connection(), None)

    def test_connected_fallback(self):
